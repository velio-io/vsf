...
(defn sample-output-fn [_config]
  :output-result)


(deftest compile-output!-test
  (testing "custom output"
    (let [io-compiled (sut/compile-output!
                       :foo
                       {:type    :custom
                        :builder 'vsf.core-test/sample-output-fn
                        :config  {:path "/tmp/foo"}})]
      (is (= (:component io-compiled)
             :output-result)))))


(deftest config-keys-test
  (is (= #{:foo :bar}
         (sut/config-keys {:foo {} :bar {}})))
  (is (= #{}
         (sut/config-keys {}))))
...
(deftest new-config-test
  (testing "same config"
    (let [old-config {:foo {} :bar {}}
          new-config {:foo {} :bar {}}]
      (is (empty? (:to-remove (sut/new-config old-config new-config))))
      (is (empty? (:to-add (sut/new-config old-config new-config))))
      (is (empty? (:to-reload (sut/new-config old-config new-config))))))
  (testing "to-add"
    (let [old-config {:foo {} :bar {}}
          new-config {:foo {} :bar {} :baz {}}]
      (is (empty? (:to-remove (sut/new-config old-config new-config))))
      (is (= #{:baz} (:to-add (sut/new-config old-config new-config))))
      (is (empty? (:to-reload (sut/new-config old-config new-config))))))
  (testing "to-reload"
    (let [old-config {:foo {} :bar {}}
      ...
      (is (empty? (:to-remove (sut/new-config old-config new-config))))
      (is (= #{:baz} (:to-add (sut/new-config old-config new-config))))
      (is (= #{:bar} (:to-reload (sut/new-config old-config new-config))))))
  (testing "to-remove"
    (let [old-config {:foo {} :bar {}}
          new-config {:foo {}}]
      (is (= #{:bar} (:to-remove (sut/new-config old-config new-config))))
      (is (empty? (:to-add (sut/new-config old-config new-config))))
      (is (empty? (:to-reload (sut/new-config old-config new-config)))))))


(deftest stream-component-test
  (let [initial-streams {:foo {:actions {:action   :fixed-event-window
                                         :params   [{:size 100}]
                                         :children []}}
        ...
        new-streams     {:bar {:actions {:action   :above-dt
                         ...
                         :baz {:actions {:action   :fixed-event-window
                                         :params   [{:size 200}]
                                         :children []}}}
        handler         (sut/stream-handler {:streams-configurations initial-streams})]
    (let [{:keys [streams streams-configurations]} (-> (.state handler) deref)]
      (is (= streams-configurations initial-streams))
      (is (= (set (keys streams)) #{:foo :bar})))

    (let [{:keys [streams streams-configurations]} (sut/reload handler new-streams)]
      (is (= streams-configurations new-streams))
      (is (= (set (keys streams)) #{:bar :baz})))))
          ...
          stream   {:description "foo"
                    :actions     {:action   :io
                                  :children [{:action :test-action
                                             ...
                                             {:action :test-action
                                              :params [recorder]}]}}
          {:keys [entrypoint]} (sut/compile-stream!
                                {}
                                stream)]
      (is (fn? entrypoint))
      (entrypoint {:metric 12})
...
(deftest index-test
  (let [index         (index/index)
        ...
        pubsub-result (atom [])
        {:keys [entrypoint]} (sut/compile-stream! {:index         index
                                                   :pubsub        pubsub
  ...
  (let [stream {:name        "my-stream"
                ...
                :actions     (a/sdo
                              ...
                              (a/where [:and
                                        [:< :metric 10]
                                        [:> :metric 1]]))}
        {:keys [entrypoint]} (sut/compile-stream! {:source-stream "my-stream"}
                                                  stream)]
    (is (fn? entrypoint))
