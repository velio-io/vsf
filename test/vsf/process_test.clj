...
(deftest keep-non-discarded-events-test
  (is (nil? (sut/keep-non-discarded-events {:tags ["vsf/discard"]})))
  (is (= {:tags ["foo"]} (sut/keep-non-discarded-events {:tags ["foo"]})))
  (is (= [{:tags ["foo"]}] (sut/keep-non-discarded-events [{:tags ["foo"]}])))
  (is (nil? (sut/keep-non-discarded-events [{:tags ["vsf/discard"]}])))
  (is (= [{:tags ["ok"]}]
         (sut/keep-non-discarded-events [{:tags ["vsf/discard"]} {:tags ["ok"]}]))))


(deftest where*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/where* nil [:pos? :metric] rec)
                  state
                  [{:metric 10} {:metric -1}]
                  [{:metric 10}])
    (test-actions (sut/where* nil [:pos? [:foo :metric]] rec)
                  state
    ...
    (test-actions (sut/where* nil [:pos? :metric] rec)
                  ...
                  [{:metric 30}])
    (test-actions (sut/where* nil [:> :metric 20] rec)
                  state
                  [{:metric -1} {:metric 30} {:metric 0}]
                  [{:metric 30}])
    (test-actions (sut/where* nil [:> [:foo :bar :metric] 20] rec)
                  state
    ...
    (test-actions (sut/where* nil
                  ...
                  [{:host "foo" :metric 2}
                   {:host "foobar" :metric 2}])
    (test-actions (sut/where* nil [:contains :tags "foo"] rec)
                  state
                  [{:metric 10} {:metric 10 :tags ["foo"]} {:tags []}]
                  [{:metric 10 :tags ["foo"]}])
    (test-actions (sut/where* nil [:absent :tags "foo"] rec)
                  state
                  [{:metric 10} {:metric 10 :tags ["foo"]} {:tags []}]
                  [{:metric 10} {:tags []}])
    (test-actions (sut/where* nil [:and
                                   [:or
                                    [:pos? :metric]
                                    [:= :metric -1]]
                                   [:and
...
(deftest increment*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/increment* nil rec)
                  state
                  [{:metric 10} {:metric 11}]
                  [{:metric 11} {:metric 12}])))
...
(deftest fixed-event-window*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/fixed-event-window* nil {:size 3} rec)
                  state
                  [{:metric 10} {:metric 11}]
                  [])
    (test-actions (sut/fixed-event-window* nil {:size 3} rec)
...
(deftest coll-mean*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-mean* nil rec)
                  state
                  [[{:metric 10}]]
                  [{:metric 10}])
    (test-actions (sut/coll-mean* nil rec)
...
(deftest expired-test
  (let [[rec state] (recorder)]
    (test-actions (sut/expired* nil rec)
                  state
                  [{:state "expired"}
                   {:state "ok"}
                   {:time 60}
...
(deftest not-expired-test
  (let [[rec state] (recorder)]
    (test-actions (sut/not-expired* nil rec)
                  state
                  [{:state "expired"}
                   {:state "ok"}
                   {:time 1}
...
(deftest cond-dt*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/cond-dt* nil [:> :metric 10] 10 rec)
                  state
                  [{:time 1 :metric 12}
                   {:time 4 :metric 12}
                   {:metric 12}
...
(deftest critical*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/critical* nil rec)
                  state
                  [{:state "ok" :metric 1}
...
(deftest default*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/default* nil :state "ok" rec)
                  state
                  [{:metric 1}
                   {:state "critical" :metric 2}
                   {:state "critical" :metric 3}
...
(deftest coalesce*-test
  (let [state  (atom [])
        ...
        action (sut/coalesce* nil {:duration 5 :fields [:host :service]} rec)]
  ...
  (let [state  (atom [])
        rec    (fn [event] (swap! state conj event))
        action (sut/coalesce* nil {:duration 5 :fields [:host [:nested :service]]} rec)]
...
(deftest with*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/with* nil {:state "ok"} rec)
                  state
                  [{:metric 1}
                   {:state "critical" :metric 2}
                   {:state "critical" :metric 3}
...
(deftest coll-max*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-max* nil rec)
                  state
                  [[{:metric 1}
                    {:metric 10}
                    {}
...
(deftest coll-quotient*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-quotient* nil rec)
                  state
                  [[{:metric 1}
                    {:metric 10}
                    {:metric 5}]]
...
(deftest coll-sum*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-sum* nil rec)
                  state
                  [[{:metric 1}
...
(deftest coll-min*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-min* nil rec)
                  state
                  [[{:metric 1}
                    {:metric 10}
                    {}
...
(deftest coll-rate*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-rate* nil rec)
                  state
                  [[{:metric 1 :time 1}]]
                  [{:metric 1 :time 1}])
    (test-actions (sut/coll-rate* nil rec)
...
(deftest sflatten*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/sflatten* nil rec)
                  state
                  [[{:metric 1 :time 1}
                    {:metric 10 :time 2}
                    {:metric 4 :time 3}
...
(deftest tag*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/tag* nil "foo" rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 2 :tags ["foo"]}
                  ...
                  [{:metric 1 :time 1 :tags ["foo"]}
                   ...
                   {:metric 5 :time 4 :tags ["foo"]}]))
  (let [[rec state] (recorder)]
    (test-actions (sut/tag* nil ["foo" "bar"] rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 2 :tags ["foo"]}
                   {:metric 10 :time 1 :tags ["bar"]}
...
(deftest untag*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/untag* nil "foo" rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 2 :tags ["foo"]}
                   {:metric 10 :time 1 :tags ["bar"]}
    ...
    (test-actions (sut/untag* nil ["foo" "bar"] rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 2 :tags ["foo"]}
                   {:metric 10 :time 1 :tags ["bar"]}
...
(deftest dtt*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/ddt* nil false rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 10 :time 4}
                   {:metric 11 :time 7}
                  ...
                  [{:metric (/ 9 3) :time 4}
                   {:metric (/ 1 3) :time 7}
                   {:metric -1 :time 10}])
    (test-actions (sut/ddt* nil true rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 10 :time 4}
                   {:metric 11 :time 7}
...
(deftest scale*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/scale* nil 10 rec)
                  state
                  [{:metric 1 :time 1}
                   {:metric 10 :time 4}]
...
(deftest fixed-time-window*-test
  (testing "no delay"
    (let [[rec state] (recorder)]
      (test-actions (sut/aggregation* nil
...
(deftest moving-event-window*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/moving-event-window* nil
                                            {:size 5}
                                            rec)
                  state
                  [{:metric 1 :time 0 :state "ok"}
...
(deftest ewma-timeless*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/ewma-timeless* nil
                                      0
                                      rec)
                  state
                  [{:metric 1 :time 0 :state "ok"}
...
(deftest warning*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/warning* nil rec)
                  state
                  [{:state "ok" :metric 1}
...
(deftest over*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/over* nil 10 rec)
                  state
                  [{:metric 10}
                   {:metric 1}
                   {:metric 4}
...
(deftest under*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/under* nil 10 rec)
                  state
                  [{:metric 10}
                   {:metric 1}
                   {:metric 4}
...
(deftest changed*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/changed* nil {:field :state :init "ok"} rec)
                  state
                  [{:metric 1 :state "ok"}
                   {:metric 2 :state "ok"}
                  ...
                  [{:metric 3 :state "critical"}
                   ...
                   {:metric 6 :state "critical"}]))
  (let [[rec state] (recorder)]
    (test-actions (sut/changed* nil {:field [:nested :state] :init "ok"} rec)
                  state
...
(deftest count*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-count* nil
                                   rec)
                  state
                  [[]
                   [{}]
...
(deftest coll-percentiles*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-percentiles* nil
                                         [0 0.5 1]
                                         rec)
                  state
...
(deftest tagged-all*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/tagged-all* nil
                                   "foo"
                                   rec)
                  state
...
(deftest json-fields*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/json-fields* nil
                                    [:foo]
                                    rec)
                  state
                  [{:foo "{\"bar\": \"baz\"}"}]
...
(deftest exception-stream*-test
  (let [[rec state] (recorder)
        stream (sut/exception-stream* nil
                                      (fn [_]
                                        (throw (ex-info "boom" {})))
                                      rec)]
    (stream {:foo 1})
    ...
    (is (= "vsf-exception" (:service (first @state))))
    (is (instance? Exception (:exception (first @state))))))


(deftest to-base64*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/to-base64* nil [:host :service] rec)
                  state
                  [{:host "aa" :service "bb"}
...
(deftest from-base64*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/from-base64* nil [:host :service] rec)
                  state
                  [{:host "YWE=" :service "YmI="}
                   {:host "YmI=" :service "YWE=" :state "critical"}]
                  [{:host "aa" :service "bb"}
...
(deftest sformat*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/sformat* nil "%s-cc-%s" :format-test [:host :service] rec)
                  state
                  [{:host "aa" :service "bb"}
                   {:host "bb" :service "aa" :state "critical"}]
  ...
  (let [[rec state] (recorder)]
    (test-actions (sut/sformat* nil "%s-cc" :service [:host] rec)
                  state
                  [{:host "aa" :service "bb"}]
...
(deftest coll-top-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-top* nil 2 rec)
                  state
...
(deftest coll-bottom-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-bottom* nil 2 rec)
                  state
                  [[{:metric 10} {:metric 4} {:metric 100} {:metric 2}]
                   [{:metric 10}]
                   [{:metric -10} {:metric 1} {:metric 2} {:metric 0}]]
...
(deftest stable*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/stable* nil 10 :state rec)
                  state
                  ...
                  [{:state "critical" :time 1}
                   ...
                   {:state "ok" :time 41}
                   {:state "ok" :time 52}])
    (test-actions (sut/stable* nil 10 [:nested :state] rec)
                  state
...
(deftest rename-keys*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/rename-keys* nil {:host        :service
                                         :environment :env} rec)
                  state
...
(deftest keep-keys*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/keep-keys* nil [:host :service :metric] rec)
                  state
                  ...
                  [{:host "foo" :service "bar" :metric 10}
                   ...
                   {:service "bar"}
                   {}])
    (test-actions (sut/keep-keys* nil [[:foo :bar :baz] :a] rec)
                  state
...
(deftest coll-where*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-where* nil [:pos? :metric] rec)
                  state
                  [[{:metric 10} {:metric 10}]]
                  [[{:metric 10} {:metric 10}]])
    (test-actions (sut/coll-where* nil [:pos? :metric] rec)
                  ...
                  [[{:metric -1} {:metric 30} {:metric 0}]]
                  [[{:metric 30}]])
    (test-actions (sut/coll-where* nil [:> :metric 20] rec)
                  state
                  [[{:metric -1} {:metric 30} {:metric 0}]]
                  [[{:metric 30}]])
    (test-actions (sut/coll-where* nil
                                   [:and
                                    [:> :metric 20]
                                    [:< :metric 40]]
                                   rec)
...
(deftest coll-sort*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-sort* nil :time rec)
                  state
...
(deftest moving-time-window*
  (let [[rec state] (recorder)]
    (test-actions (sut/moving-time-window* nil {:duration 5} rec)
                  state
                  [{:time 3}
                   {:time 5}
                   {:time 7}
    ...
    (test-actions (sut/moving-time-window* nil {:duration 3} rec)
                  state
                  [{:time 1}
                   {:time 2}
                   {:time 3}
...
(deftest ssort*-test
  (let [[rec state] (recorder)]
    (test-actions (sut/aggregation* nil {:duration 5 :field :time :aggr-fn :ssort :delay 10} rec)
                  state
                  [{:time 0}
                   {:time 3}
                   {:time 2}
  ...
  (let [[rec state] (recorder)]
    (test-actions (sut/aggregation* nil {:duration 5 :field :time :aggr-fn :ssort :delay 10} rec)
                  ...
                  [{:time 1}
                   ...
                   {:time 30}]))
  (let [[rec state] (recorder)]
    (test-actions (sut/aggregation* nil {:duration 10 :field :time :aggr-fn :ssort :delay 20} rec)
                  state
                  [{:time 1}
                   {:time 10}
                   {:time 4}
                  ...
                  [{:time 1}
                   ...
                   {:time 13}]))
  (let [[rec state] (recorder)]
    (test-actions (sut/aggregation* nil {:duration 10 :field [:nested :a] :aggr-fn :ssort :nested? true} rec)
                  state
                  [{:time 1 :nested {:a 4}}
                   {:time 3 :nested {:a 1}}
                   {:time 7 :nested {:a -1}}
...
(deftest coll-increase*
  (let [[rec state] (recorder)]
    (test-actions (sut/coll-increase* nil rec)
                  state
                  [[{:time 1 :metric 10}
                    {:time 2 :metric 20}
                    {:time 11 :metric 50}]
...
(deftest extract-test
  (let [[rec state] (recorder)]
    (test-actions (sut/extract* nil :foo rec)
                  state
                  [{:foo {:time 1}}
                   {:foo {:time 2} :bar "a"}
                   {:foo nil}
...
(deftest percentiles
  (let [[rec state] (recorder)]
    (test-actions (sut/percentiles* nil
